...
import httpx
from config import API_ID, API_HASH, BOT_TOKEN, ESPRO_API_KEY, ESPRO_API_URL, DEFAULT_MODEL
...
async def get_ai_reply(user_message):
    ...
    headers = {
        "Authorization": f"Bearer {ESPRO_API_KEY}",
        "Content-Type": "application/json"
    }

    async with httpx.AsyncClient(timeout=30) as client:
        try:
            response = await client.post(ESPRO_API_URL, headers=headers, json=payload)
...
if __name__ == "__main__":
    print("🤖 Espro AI Chat Bot is running...")
    app.run()
